...
const DUMMY_PORTFOLIO = [
  { id: "prog-1", name: "Alpha Initiative", rag: "green", schedule: "green", budget: "green", risk: "amber", csat: 85, progress: 75, nextMilestone: 12 },
  { id: "prog-2", name: "Beta Expansion", rag: "amber", schedule: "amber", budget: "green", risk: "amber", csat: 60, progress: 40, nextMilestone: 2 },
  { id: "prog-3", name: "Gamma Integration", rag: "red", schedule: "red", budget: "red", risk: "red", csat: 40, progress: 15, nextMilestone: -4 },
];
              ...
              <tbody className="divide-y divide-white/5">
                {DUMMY_PORTFOLIO.map(p => (
                  <tr key={p.id} className="hover:bg-white/5 transition-colors group cursor-pointer">
                    ...
                    {['rag', 'schedule', 'budget', 'risk'].map((key) => {
                      const val = p[key as keyof typeof p] as string;
                      return (
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        <span className={cn("font-medium", p.csat > 80 ? "text-green-400" : p.csat > 50 ? "text-amber-400" : "text-red-400")}>
                          {p.csat}%
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        <span className={cn("font-medium", p.nextMilestone < 0 ? "text-red-400" : "text-white")}>
                          {p.nextMilestone < 0 ? `${Math.abs(p.nextMilestone)}d late` : `in ${p.nextMilestone}d`}
                        </span>
                      </div>
                    </td>
